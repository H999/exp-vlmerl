...
def _wrap(fn, i, args, error_queue, res_queue, stop_event):
...
def start_processes(fn, args=(), nprocs=1, join=True, daemon=False, start_method="spawn"):
    ...
    for i in range(nprocs):
        ...
        res_queue = mp.SimpleQueue()
        process = mp.Process(
            target=_wrap,
            args=(fn, i, args, error_queue, res_queue, stop_event),
            daemon=daemon,
        )
        process.start()
        error_queues.append(error_queue)
        res_queues.append(res_queue)
        processes.append(process)

    if not join: return ProcessContext(processes, error_queues), res_queues

    res = []
    if join:
        for process, error_queue, res_queue in zip(processes, error_queues, res_queues):
            process.join()

            if not error_queue.empty():
                error_msg = error_queue.get()
                print(f"Process encountered an error: {error_msg}")
                res.append(None)
                continue

            rq = res_queue.get()
            res.append(rq if rq else None)
...
# mod the https://stackoverflow.com/a/53180921 for using spawn
...
class NoDaemonContext(multiprocessing.context.SpawnContext):
